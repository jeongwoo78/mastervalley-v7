// ========================================
// Mouvements artistiques — Français (fr)
// Structure i18n · 9 lignes 2 paragraphes (1er 5 lignes 3+2 / 2e 4 lignes 2+2)
// 2026-03-11 수정 반영본
// ========================================

// ========== Informations de base ==========
export const movementsBasicInfo = {

  // ===== 1er Chargement (niveau mouvement) =====
  'ancient': {
    loading: {
      name: 'Grèce et Rome antiques (Ancient Greek & Roman, 800 av. J.-C.–500 apr. J.-C.)',
      subtitle1: 'Sculpture grecque · Mosaïque romaine',
      subtitle2: 'La beauté idéale des dieux et des héros'
    }
  },
  'medieval': {
    loading: {
      name: 'Art médiéval (Medieval Art, IVe–XIVe siècle)',
      subtitle1: 'Byzantin · Gothique · Miniature islamique',
      subtitle2: 'L\'art de lumière né de la foi'
    }
  },
  'renaissance': {
    loading: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'De Vinci · Michel-Ange · Botticelli',
      subtitle2: 'Redécouvrir le monde avec des yeux humains'
    }
  },
  'baroque': {
    loading: {
      name: 'Baroque (Baroque, XVIIe siècle)',
      subtitle1: 'Le Caravage · Rembrandt · Vélasquez',
      subtitle2: 'Le drame du clair-obscur'
    }
  },
  'rococo': {
    loading: {
      name: 'Rococo (Rococo, XVIIIe siècle)',
      subtitle1: 'Watteau · Boucher · Fragonard',
      subtitle2: 'Le jeu élégant de l\'aristocratie'
    }
  },
  'neoclassicism_vs_romanticism_vs_realism': {
    loading: {
      name: 'Néoclassicisme · Romantisme · Réalisme (XVIIIe–XIXe siècle)',
      subtitle1: 'David · Delacroix · Courbet',
      subtitle2: 'Raison, émotion et réalité divergent'
    }
  },
  'impressionism': {
    loading: {
      name: 'Impressionnisme (Impressionism, 1860–1890)',
      subtitle1: 'Monet · Renoir · Degas',
      subtitle2: 'L\'instant où la lumière devient couleur'
    }
  },
  'postImpressionism': {
    loading: {
      name: 'Postimpressionnisme (Post-Impressionism, 1880–1910)',
      subtitle1: 'Van Gogh · Cézanne · Gauguin',
      subtitle2: 'Au-delà de la lumière, vers l\'intérieur'
    }
  },
  'fauvism': {
    loading: {
      name: 'Fauvisme (Fauvism, 1905–1910)',
      subtitle1: 'Matisse · Derain · Vlaminck',
      subtitle2: 'Les fauves qui ont libéré la couleur'
    }
  },
  'expressionism': {
    loading: {
      name: 'Expressionnisme (Expressionism, 1905–1930)',
      subtitle1: 'Munch · Kirchner · Kokoschka',
      subtitle2: 'Peindre ce que l\'on ressent'
    }
  },
  'modernism': {
    loading: {
      name: 'Modernisme (Modernism, XXe siècle)',
      subtitle1: 'Picasso · Magritte · Chagall',
      subtitle2: 'La destruction de toutes les règles'
    }
  },

  // ===== 2e Résultat (niveau artiste/style) =====

  // ----- Grèce et Rome antiques -----
  'classical-sculpture': {
    result: {
      name: 'Grèce et Rome antiques (Ancient Greek & Roman, 800 av. J.-C.–500 apr. J.-C.)',
      subtitle1: 'Sculpture grecque (Classical Sculpture)',
      subtitle2: 'L\'origine de la proportion parfaite'
    }
  },
  'roman-mosaic': {
    result: {
      name: 'Grèce et Rome antiques (Ancient Greek & Roman, 800 av. J.-C.–500 apr. J.-C.)',
      subtitle1: 'Mosaïque romaine (Roman Mosaic)',
      subtitle2: 'Des récits gravés dans la pierre'
    }
  },

  // ----- Médiéval -----
  'byzantine': {
    result: {
      name: 'Art médiéval (Medieval Art, IVe–XIVe siècle)',
      subtitle1: 'Byzantin (Byzantine)',
      subtitle2: 'Le monde doré du divin'
    }
  },
  'gothic': {
    result: {
      name: 'Art médiéval (Medieval Art, IVe–XIVe siècle)',
      subtitle1: 'Gothique (Gothic)',
      subtitle2: 'Le ciel bâti de lumière'
    }
  },
  'islamic-miniature': {
    result: {
      name: 'Art médiéval (Medieval Art, IVe–XIVe siècle)',
      subtitle1: 'Miniature islamique (Islamic Miniature)',
      subtitle2: 'La finesse des joyaux'
    }
  },

  // ----- Renaissance -----
  'leonardo': {
    result: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'Léonard de Vinci (Leonardo da Vinci)',
      subtitle2: 'Science et art réunis'
    }
  },
  'michelangelo': {
    result: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'Michel-Ange (Michelangelo)',
      subtitle2: 'Le sculpteur qui insuffla la vie à la peinture'
    }
  },
  'raphael': {
    result: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'Raphaël (Raphael)',
      subtitle2: 'Le peintre de l\'équilibre parfait'
    }
  },
  'botticelli': {
    result: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'Botticelli',
      subtitle2: 'Le poète qui chanta la beauté'
    }
  },
  'titian': {
    result: {
      name: 'Renaissance (Renaissance, XIVe–XVIe siècle)',
      subtitle1: 'Titien (Titian)',
      subtitle2: 'Construire la forme par la couleur'
    }
  },

  // ----- Baroque -----
  'caravaggio': {
    result: {
      name: 'Baroque (Baroque, XVIIe siècle)',
      subtitle1: 'Le Caravage (Caravaggio)',
      subtitle2: 'Un rayon de lumière dans les ténèbres'
    }
  },
  'rembrandt': {
    result: {
      name: 'Baroque (Baroque, XVIIe siècle)',
      subtitle1: 'Rembrandt',
      subtitle2: 'Éclairer l\'âme par la lumière'
    }
  },
  'velazquez': {
    result: {
      name: 'Baroque (Baroque, XVIIe siècle)',
      subtitle1: 'Vélasquez (Velázquez)',
      subtitle2: 'Le peintre des peintres'
    }
  },
  'rubens': {
    result: {
      name: 'Baroque (Baroque, XVIIe siècle)',
      subtitle1: 'Rubens',
      subtitle2: 'Dynamisme et abondance'
    }
  },

  // ----- Rococo -----
  'watteau': {
    result: {
      name: 'Rococo (Rococo, XVIIIe siècle)',
      subtitle1: 'Watteau',
      subtitle2: 'Le début de la fête galante'
    }
  },
  'boucher': {
    result: {
      name: 'Rococo (Rococo, XVIIIe siècle)',
      subtitle1: 'Boucher',
      subtitle2: 'Peindre la splendeur de la cour'
    }
  },

  // ----- Néoclassicisme · Romantisme · Réalisme -----
  'david': {
    result: {
      name: 'Néoclassicisme (Neoclassicism, XVIIIe–XIXe siècle)',
      subtitle1: 'Jacques-Louis David',
      subtitle2: 'Le peintre de la Révolution'
    }
  },
  'ingres': {
    result: {
      name: 'Néoclassicisme (Neoclassicism, XVIIIe–XIXe siècle)',
      subtitle1: 'Jean-Auguste-Dominique Ingres',
      subtitle2: 'La quête de la ligne parfaite'
    }
  },
  'delacroix': {
    result: {
      name: 'Romantisme (Romanticism, XIXe siècle)',
      subtitle1: 'Eugène Delacroix',
      subtitle2: 'Peindre la tempête des émotions'
    }
  },
  'turner': {
    result: {
      name: 'Romantisme (Romanticism, XIXe siècle)',
      subtitle1: 'J.M.W. Turner',
      subtitle2: 'Le peintre de la lumière et de l\'atmosphère'
    }
  },
  'courbet': {
    result: {
      name: 'Réalisme (Realism, XIXe siècle)',
      subtitle1: 'Gustave Courbet',
      subtitle2: 'Ne peindre que ce que l\'on voit'
    }
  },
  'manet': {
    result: {
      name: 'Réalisme (Realism, XIXe siècle)',
      subtitle1: 'Édouard Manet',
      subtitle2: 'Ouvrir la porte de la peinture moderne'
    }
  },

  // ----- Impressionnisme -----
  'monet': {
    result: {
      name: 'Impressionnisme (Impressionism, 1860–1890)',
      subtitle1: 'Claude Monet',
      subtitle2: 'Poursuivre l\'instant de lumière'
    }
  },
  'renoir': {
    result: {
      name: 'Impressionnisme (Impressionism, 1860–1890)',
      subtitle1: 'Pierre-Auguste Renoir',
      subtitle2: 'Peindre les instants de bonheur'
    }
  },
  'degas': {
    result: {
      name: 'Impressionnisme (Impressionism, 1860–1890)',
      subtitle1: 'Edgar Degas',
      subtitle2: 'Saisir l\'instant du mouvement'
    }
  },
  'caillebotte': {
    result: {
      name: 'Impressionnisme (Impressionism, 1860–1890)',
      subtitle1: 'Gustave Caillebotte',
      subtitle2: 'Le quotidien parisien sur la toile'
    }
  },

  // ----- Postimpressionnisme -----
  'vangogh': {
    result: {
      name: 'Postimpressionnisme (Post-Impressionism, 1880–1910)',
      subtitle1: 'Vincent van Gogh',
      subtitle2: 'La passion du pinceau en spirale'
    }
  },
  'cezanne': {
    result: {
      name: 'Postimpressionnisme (Post-Impressionism, 1880–1910)',
      subtitle1: 'Paul Cézanne',
      subtitle2: 'Construire la nature par la structure'
    }
  },
  'gauguin': {
    result: {
      name: 'Postimpressionnisme (Post-Impressionism, 1880–1910)',
      subtitle1: 'Paul Gauguin',
      subtitle2: 'Quitter la civilisation en quête de pureté'
    }
  },

  // ----- Fauvisme -----
  'matisse': {
    result: {
      name: 'Fauvisme (Fauvism, 1905–1910)',
      subtitle1: 'Henri Matisse',
      subtitle2: 'Le magicien de la couleur'
    }
  },
  'derain': {
    result: {
      name: 'Fauvisme (Fauvism, 1905–1910)',
      subtitle1: 'André Derain',
      subtitle2: 'L\'explosion de la couleur'
    }
  },
  'vlaminck': {
    result: {
      name: 'Fauvisme (Fauvism, 1905–1910)',
      subtitle1: 'Maurice de Vlaminck',
      subtitle2: 'Faire exploser la couleur par instinct'
    }
  },

  // ----- Expressionnisme -----
  'munch': {
    result: {
      name: 'Expressionnisme (Expressionism, 1905–1930)',
      subtitle1: 'Edvard Munch',
      subtitle2: 'Peindre le cri intérieur'
    }
  },
  'kirchner': {
    result: {
      name: 'Expressionnisme (Expressionism, 1905–1930)',
      subtitle1: 'Ernst Ludwig Kirchner',
      subtitle2: 'Révéler l\'angoisse urbaine'
    }
  },
  'kokoschka': {
    result: {
      name: 'Expressionnisme (Expressionism, 1905–1930)',
      subtitle1: 'Oskar Kokoschka',
      subtitle2: 'Le portrait comme champ de bataille psychologique'
    }
  },

  // ----- Modernisme -----
  'picasso': {
    result: {
      name: 'Cubisme (Cubism, début du XXe siècle)',
      subtitle1: 'Pablo Picasso',
      subtitle2: 'Le révolutionnaire qui déconstruisit le regard'
    }
  },
  'lichtenstein': {
    result: {
      name: 'Pop Art (Pop Art, 1950–1970)',
      subtitle1: 'Roy Lichtenstein',
      subtitle2: 'L\'homme qui transforma la bande dessinée en art'
    }
  },
  'miro': {
    result: {
      name: 'Surréalisme (Surrealism, 1920–1960)',
      subtitle1: 'Joan Miró',
      subtitle2: 'L\'émotion avant la forme'
    }
  },
  'magritte': {
    result: {
      name: 'Surréalisme (Surrealism, 1920–1960)',
      subtitle1: 'René Magritte',
      subtitle2: 'Le piège du visible'
    }
  },
  'chagall': {
    result: {
      name: 'Surréalisme (Surrealism, 1920–1960)',
      subtitle1: 'Marc Chagall',
      subtitle2: 'Poète de l\'amour et des rêves'
    }
  }
};

// ========== 1re Éducation : Chargement (introduction au mouvement) ==========
export const movementsLoadingEducation = {
  // ===== Antiquité — Grèce et Rome =====
  'ancient': {
    name: 'Grèce et Rome antiques',
    description: `Un art à la recherche de la beauté idéale des dieux et des héros naquit en Méditerranée.
La sculpture grecque incarna le corps humain idéal grâce au nombre d\'or et au contrapposto.
La mosaïque romaine assembla des milliers de tesselles colorées pour illustrer mythes et scènes du quotidien.

Proportion et équilibre parfaits : voici le fondement de deux mille ans d\'art occidental.
Redécouvert à la Renaissance, il devint la référence de tout l\'art occidental qui suivit.`
  },

  // ===== Médiéval =====
  'medieval': {
    name: 'Art médiéval',
    description: `Du IVe au XIVe siècle, les églises d\'Europe et les mosquées du Moyen-Orient se parèrent d\'or et d\'ornements sacrés.
Byzance exprima la gloire divine par des mosaïques dorées, le Gothique la lumière céleste par des vitraux.
L\'Islam captura la beauté courtisane dans des miniatures aux couleurs de joyaux.

Une ère qui incarna le sacré à travers des images planes et symboliques.
Mille ans d\'esthétique où foi et décoration se fondirent en un.`
  },

  // ===== Renaissance =====
  'renaissance': {
    name: 'Renaissance',
    description: `Dans l\'Italie du XIVe siècle débuta une nouvelle exploration qui replaça l\'homme et la nature au centre.
De Vinci obtint un clair-obscur mystérieux grâce au sfumato ; Michel-Ange sculpta le corps héroïque.
Botticelli tissa les mythes en lignes fluides ; Titien ouvrit le monde des sens par ses couleurs dorées.

La perspective et l\'anatomie s\'unirent à la peinture, et l\'art devint science de l\'observation et de la raison.
La renaissance des lettres et des arts qui traversa le Moyen Âge vers la modernité : l\'ère où l\'on commença à redessiner le monde avec des yeux humains.`
  },

  // ===== Baroque =====
  'baroque': {
    name: 'Baroque',
    description: `Au XVIIe siècle, au cœur des tourments de la Réforme, le clair-obscur dramatique domina la toile.
Le Caravage fit jaillir un rayon de lumière dans les ténèbres les plus profondes et inventa le ténébrisme.
Rembrandt fixa l\'époque avec son clair-obscur doré ; Vélasquez avec sa magie spatiale.

Les peintres répondirent par la lumière et l\'ombre à la demande de l\'Église d\'émouvoir les fidèles.
Le Baroque est un art né de l\'ère de la religion et du pouvoir, où l\'émotion et le drame éclatèrent.`
  },

  // ===== Rococo =====
  'rococo': {
    name: 'Rococo',
    description: `Au XVIIIe siècle, autour du château de Versailles, le jeu élégant de la société aristocratique s\'épanouit.
Watteau peignit les fêtes galantes des nobles ; Boucher, les fantaisies rosées des déesses.
Une ère où les tons pastel et les courbes gracieuses envahirent la toile.

Au lieu du drame pesant du Baroque, la légèreté et le plaisir dominèrent l\'art.
Le Rococo orna le dernier âge d\'or de la culture aristocratique, juste avant la Révolution française.`
  },

  // ===== Néoclassicisme vs Romantisme vs Réalisme =====
  'neoclassicism_vs_romanticism_vs_realism': {
    name: 'Néoclassicisme · Romantisme · Réalisme',
    description: `À la fin du XVIIIe siècle s\'ouvrit une ère où raison, émotion et réalité entrèrent violemment en collision.
David et Ingres poursuivirent la beauté idéale de l\'Antiquité ; Delacroix et Turner déversèrent des tempêtes d\'émotion sur la toile.
Courbet et Manet rejetèrent l\'idéal comme la fantaisie et ne présentèrent que la réalité devant leurs yeux.

Néoclassicisme, Romantisme, Réalisme — trois courants dont la collision ouvrit la porte de l\'art moderne.`
  },

  // ===== Impressionnisme =====
  'impressionism': {
    name: 'Impressionnisme',
    description: `En 1874, la moquerie « ce n\'est pas de la peinture, c\'est juste une impression » devint le nom d\'une révolution.
Monet captura l\'instant de la lumière ; Renoir, la couleur du bonheur ; Degas, l\'instant du mouvement.
Caillebotte fixa les rues de Paris avec un regard aussi précis que la photographie.

Les premiers peintres à quitter l\'atelier pour peindre sous la lumière du soleil.
Le début de la révolution picturale où la lumière et la couleur remplacèrent la forme.`
  },

  // ===== Postimpressionnisme =====
  'postImpressionism': {
    name: 'Postimpressionnisme',
    description: `Au-delà de la lumière instantanée captée par l\'Impressionnisme, des peintres explorèrent structure, forme et émotion.
Van Gogh sonda son intérieur par des touches en spirale ; Cézanne poursuivit la structure de la nature par la géométrie.
Gauguin tourna le dos à la civilisation et partit vers les couleurs primitives.

Au-delà de la lumière impressionniste, chacun ouvrit un nouveau chemin à sa manière.
Toutes les expérimentations de l\'art moderne du XXe siècle partirent d\'eux.`
  },

  // ===== Fauvisme =====
  'fauvism': {
    name: 'Fauvisme',
    description: `Au Salon de Paris de 1905, la critique « on dirait une cage aux fauves » leur donna leur nom.
Matisse avec ses aplats audacieux de couleurs pures, Derain et Vlaminck avec leurs primaires explosifs, brisèrent la tradition.
La peinture exprimée directement du tube rugit sur la toile.

La forme avant la couleur, la description avant l\'émotion : une révolution où la couleur devint protagoniste.
Ce mouvement, bref mais intense, ouvrit la porte à l\'Expressionnisme et à l\'art abstrait.`
  },

  // ===== Expressionnisme =====
  'expressionism': {
    name: 'Expressionnisme',
    description: `Au début du XXe siècle, l\'angoisse et la solitude intérieures débordèrent sur la toile.
Munch peignit la terreur existentielle ; Kirchner, l\'aliénation urbaine ; Kokoschka, les tourments de l\'âme.
Des peintres qui ne peignirent pas « ce que l\'on voit » mais « ce que l\'on ressent ».

Ils révélèrent la vérité intérieure de l\'être humain par des formes distordues et des couleurs violentes.
Un art de l\'âme, né dans l\'ombre de deux guerres mondiales.`
  },

  // ===== Modernisme =====
  'modernism': {
    name: 'Modernisme',
    description: `Au XXe siècle, la tradition fut détruite et une nouvelle expérimentation artistique éclata.
Picasso déconstruisit la forme ; Miró joua avec les signes ; Magritte inversa la réalité.
Lichtenstein porta la bande dessinée sur la toile ; Chagall, l\'amour et les rêves.

Cubisme, Surréalisme, Pop Art : une ère d\'expérimentation sans frontières.
La question « qu\'est-ce que l\'art ? » devint elle-même de l\'art.`
  }
};

// ========== 2e Éducation : Résultat (style/artiste individuel) ==========
export const movementsResultEducation = {
  // ===== Antiquité — Grèce et Rome =====
  'classical-sculpture': {
    name: 'Sculpture grecque',
    description: `La beauté sculpturale de la Grèce antique et la texture du marbre ont été appliquées.
La posture du contrapposto déplace le centre de gravité, et le nombre d\'or compose mathématiquement le corps humain.

Les proportions idéales de la Vénus de Milo et la tension dramatique du Groupe du Laocoon sont gravées dans le marbre pur.
La sculpture grecque est le point de départ de l\'art occidental, qui trouva la beauté parfaite dans le corps humain.`
  },
  'roman-mosaic': {
    name: 'Mosaïque romaine',
    description: `La technique précise des tesselles romaines et leurs couleurs vibrantes ont été appliquées.
Des milliers de petites pierres colorées s\'assemblent une à une pour créer des scènes mythologiques et quotidiennes.

De la grande bataille de la Mosaïque d\'Alexandre à la vie domestique du Cave Canem, les couleurs de deux mille ans restent vives.
La mosaïque romaine est L\'essence de l\'art décoratif antique, où chaque pièce conserve le savoir-faire de l\'artisan.`
  },

  // ===== Médiéval =====
  'byzantine': {
    name: 'Byzantin',
    description: `La mosaïque dorée byzantine et la sacralité frontale ont été appliquées.
Des figures planes et symboliques se disposent sur des fonds de feuille d\'or pour exprimer une sacralité qui transcende l\'humain.

L\'Empereur Justinien de Sainte-Sophie transmet la majesté impériale ; le Christ Pantocrator, le regard divin.
L'art byzantin est un art religieux doré qui influença profondément les icônes russes et l\'art médiéval d\'Europe occidentale.`
  },
  'gothic': {
    name: 'Gothique',
    description: `L\'élan vertical du Gothique et la magie de la lumière ont été appliqués.
La lumière colorée filtrant à travers les vitraux incarne la gloire céleste dans tout l\'espace.

L\'éclat cobalt de la Vierge Bleue de Chartres et les rayons de la rose de Notre-Dame gravent l\'aspiration au ciel.
L'art gothique fut un livre de verre qui enseigna la Bible aux fidèles analphabètes, et le mystère de la lumière elle-même.`
  },
  'islamic-miniature': {
    name: 'Miniature islamique',
    description: `Le trait délicat et l\'élégance courtisane des miniatures islamiques ont été appliqués.
Des couleurs raffinées comme des joyaux et des détails complexes déploient des mondes de vie courtisane et de littérature.

Rostam et le Dragon déploie l\'épopée héroïque ; Les Amants au Jardin présente la romance courtisane en tons de joyaux.
La fleur de l\'art islamique qui inspira l\'Art nouveau et l\'Orientalisme.`
  },

  // ===== Renaissance =====
  'leonardo': {
    name: 'Léonard de Vinci',
    description: `La technique du sfumato de Léonard et son mystérieux clair-obscur ont été appliqués.
Les contours se fondent comme de la fumée, créant des transitions douces et naturelles de lumière et d\'ombre.

Le sourire de La Joconde reste une énigme après des siècles ; La Cène grava un instant de tension dramatique sur le mur.
De Vinci est L\'essence du génie universel de la Renaissance qui unifia science et art.`
  },
  'michelangelo': {
    name: 'Michel-Ange',
    description: `La beauté corporelle héroïque et le dynamisme dramatique de Michel-Ange ont été appliqués.
Avec une compréhension parfaite de l\'anatomie humaine, des figures héroïques aux proportions divines sont exprimées.

Un esprit artistique inébranlable qui peignit seul, allongé sur le dos pendant quatre ans, le plafond de la chapelle Sixtine.
Michel-Ange est un sculpteur qui libéra les anges du marbre ; un peintre qui insuffla la vie à la fresque.`
  },
  'raphael': {
    name: 'Raphaël',
    description: `L\'équilibre parfait et la composition harmonieuse de Raphaël ont été appliqués.
La beauté idéale et l\'harmonie classique s\'incarnent dans leur forme la plus pure.

Dans L\'École d\'Athènes, il réunit les philosophes antiques en un seul espace ; dans la Madone Sixtine, il atteignit le sommet de la beauté sublime.
Bien que sa vie fût brève, seulement 37 ans, il reste le sommet de la beauté idéale de la Renaissance et le fondement du classicisme ultérieur.`
  },
  'botticelli': {
    name: 'Botticelli',
    description: `La ligne élégante et le lyrisme poétique de Botticelli ont été appliqués.
Des lignes fluides et des couleurs transparentes expriment la beauté mythologique.

Les cheveux dorés au vent de La Naissance de Vénus et le jardin mystique du Printemps déploient une beauté idéale de rêve.
Botticelli est un peintre poétique qui représenta l\'âge d\'or de Florence sous le mécénat des Médicis.`
  },
  'titian': {
    name: 'Titien',
    description: `La couleur dorée et l\'opulence vénitienne du Titien ont été appliquées.
Des couleurs riches et des touches audacieuses expriment des scènes sensuelles et dramatiques.

Il montra le sommet de la couleur sensuelle dans la Vénus d\'Urbin et une composition verticale majestueuse dans L\'Assomption de la Vierge, achevant l\'école vénitienne jusqu\'à l\'âge de 90 ans.
Titien est un pionnier du colorisme qui influença profondément le Baroque et l\'Impressionnisme.`
  },

  // ===== Baroque =====
  'caravaggio': {
    name: 'Le Caravage',
    description: `Le ténébrisme du Caravage et sa saisie de l\'instant dramatique ont été appliqués.
Un seul rayon de lumière jaillit dans les ténèbres les plus profondes, révélant l\'instant décisif.

Dans La Vocation de saint Matthieu, la lumière qui déchire l\'obscurité est l\'appel divin lui-même ; dans Méduse, il gela un instant d\'horreur sur un bouclier.
Le Caravage fut L\'étincelle de l\'art baroque, qui laissa un style révolutionnaire au milieu d\'une vie de meurtre et de fuite.`
  },
  'rembrandt': {
    name: 'Rembrandt',
    description: `Le clair-obscur doré et la profonde intériorité de Rembrandt ont été appliqués.
Le contraste dramatique entre lumière et ombre crée une profondeur psychologique qui révèle les émotions et même l\'âme.

Dans La Ronde de nuit, il transforma le portrait de groupe en théâtre dramatique ; dans Le Retour du fils prodigue, il enveloppa un instant de pardon dans la lumière.
Rembrandt est un maître du portrait psychologique qui explora l\'intérieur humain à travers le clair-obscur.`
  },
  'velazquez': {
    name: 'Vélasquez',
    description: `La magie spatiale de Vélasquez et la dignité de la cour espagnole ont été appliquées.
La perspective aérienne et la touche de pinceau expriment la profondeur spatiale et le réalisme ainsi que la dignité des sujets.

Dans Les Ménines, les regards du peintre, du roi et du spectateur se croisent en un seul espace ; dans le Portrait du pape Innocent X, il pénétra l\'intérieur même d\'un homme puissant.
Vélasquez est le maître du Siècle d\'Or espagnol, dont on dit qu\'il peignit même l\'air d\'un seul coup de pinceau.`
  },
  'rubens': {
    name: 'Rubens',
    description: `La vitalité abondante et l\'énergie dynamique de Rubens ont été appliquées.
Des figures opulentes et des compositions puissantes capturent la joie de vivre sur la toile.

Dans le cycle de Marie de Médicis, il déploya l\'histoire à grande échelle ; dans Les Trois Grâces, il montra le sommet de la beauté corporelle exubérante.
Rubens est un maître du Baroque qui jouit à la fois de l\'honneur et de la richesse, laissant environ 1 400 œuvres.`
  },

  // ===== Rococo =====
  'watteau': {
    name: 'Watteau',
    description: `La fête galante et le romantisme onirique de Watteau ont été appliqués.
Les fêtes de jardin des aristocrates s\'expriment dans une atmosphère poétique et mélancolique.

Dans Pèlerinage à Cythère, il peignit le rêve des nobles de partir vers l\'île de l\'amour ; dans La Fête d\'Amour, il captura un instant de festivité élégante.
Watteau est un peintre des rêves aristocratiques qui établit le style rococo en seulement 36 ans de vie.`
  },
  'boucher': {
    name: 'Boucher',
    description: `La fantaisie rosée et la sensualité douce de Boucher ont été appliquées.
Des tons de peau rosés et des déesses flottant sur les nuages peignent des fantaisies de cour.

Dans le Portrait de Madame de Pompadour, il captura la splendeur de la femme la plus puissante de la cour ; dans Diane au bain, il perfectionna la beauté idéalisée des déesses.
Boucher est le sommet de la sensualité rococo, favorisé par Louis XV tout en poursuivant la beauté idéalisée.`
  },

  // ===== Néoclassicisme vs Romantisme vs Réalisme =====
  'david': {
    name: 'David',
    description: `Le contour net et la composition contenue de David ont été appliqués.
La forme est établie par des lignes claires et les figures héroïques sont composées avec des proportions idéales.

Dans Le Sacre de Napoléon, il fixa la grandeur de l\'histoire ; dans La Mort de Marat, la tragédie de la Révolution.
David est un peintre de la Révolution qui fixa l\'histoire par la peinture, de la Révolution française à Napoléon.`
  },
  'ingres': {
    name: 'Ingres',
    description: `Le contour lisse et la texture de porcelaine d\'Ingres ont été appliqués.
Des lignes fluides et une peau lisse expriment la pureté classique.

Dans La Grande Odalisque, il peignit un dos impossible ; dans La Baigneuse de Valpinçon, le sommet de la beauté curviligne.
Ingres est un maître du néoclassicisme qui insista sur la pureté de la ligne face à la couleur, rivalisant avec le romantique Delacroix.`
  },
  'delacroix': {
    name: 'Delacroix',
    description: `La couleur intense et la composition tourbillonnante de Delacroix ont été appliquées.
Des touches vigoureuses et des primaires vifs font éclater l\'émotion passionnée sur la toile.

Avec La Liberté guidant le peuple, il captura la ferveur révolutionnaire ; avec Les Femmes d\'Alger, des couleurs exotiques sur la toile.
Delacroix est un pionnier de la révolution de la couleur qui influença décisivement l\'Impressionnisme et le Fauvisme.`
  },
  'turner': {
    name: 'Turner',
    description: `La lumière diffuse et la texture atmosphérique de Turner ont été appliquées.
Le brouillard qui dissout les formes et les rayons dorés expriment les instants bouleversants de la nature.

Il s\'attacha au mât d\'un navire pour peindre Tempête de neige ; dans Pluie, vapeur et vitesse, il dissolut l\'énergie de la révolution industrielle dans la lumière.
Turner est un révolutionnaire de la lumière qui annonça l\'Impressionnisme en dissolvant la forme dans la lumière.`
  },
  'courbet': {
    name: 'Courbet',
    description: `La texture rugueuse au couteau et les tons sombres de Courbet ont été appliqués.
Rejetant l\'idéalisation, la réalité visible est captée dans un empâtement épais.

Il peignit le labeur rude des Casseurs de pierres et les funérailles ordinaires d\'Un enterrement à Ornans, déclarant : « Je ne peins pas les anges car je n\'en ai jamais vu. »
Courbet est le déclarant du Réalisme et un révolutionnaire qui éleva la réalité au rang d\'art.`
  },
  'manet': {
    name: 'Manet',
    description: `La composition plate audacieuse et le contraste net du clair-obscur de Manet ont été appliqués.
Éliminant hardiment les demi-teintes, sa touche assurée capte la vie de la ville moderne.

Le Déjeuner sur l\'herbe et Olympia provoquèrent de violents scandales lors de leur présentation, défiant la tradition.
Manet est le pont entre le Réalisme et l\'Impressionnisme, le lien vers l\'art moderne.`
  },

  // ===== Impressionnisme =====
  'monet': {
    name: 'Monet',
    description: `La couleur scintillante et l\'atmosphère de lumière douce de Monet ont été appliquées.
De courtes touches posent les couleurs côte à côte, laissant la lumière se mélanger dans l\'œil du spectateur.

Avec Impression, soleil levant, il donna son nom à l\'Impressionnisme ; perdant la vue à cause de la cataracte, il acheva néanmoins 250 Nymphéas.
Monet est le symbole de l\'Impressionnisme qui consacra ses 86 années de vie à la lumière.`
  },
  'renoir': {
    name: 'Renoir',
    description: `La couleur chaude et les touches douces de Renoir ont été appliquées.
Des tons de peau rosés et la lumière du soleil filtrant à travers le feuillage captent les instants de bonheur.

La foule lumineuse du Bal du Moulin de la Galette et la vie quotidienne chaleureuse du Déjeuner des canotiers.
Renoir est un peintre du bonheur qui ne lâcha jamais la beauté, même avec des pinceaux attachés à des mains déformées par les rhumatismes.`
  },
  'degas': {
    name: 'Degas',
    description: `La composition cadrée et la texture douce du pastel de Degas ont été appliquées.
Des compositions asymétriques et des angles inattendus captent les instants de mouvement.

Dans L\'Étoile, il captura des instants calculés ; dans L\'Absinthe, la solitude urbaine.
Degas est un peintre du mouvement qui ne cessa jamais de travailler même en perdant la vue.`
  },
  'caillebotte': {
    name: 'Caillebotte',
    description: `La perspective dramatique et les tons bleu-gris sereins de Caillebotte ont été appliqués.
Des vues plongeantes et une représentation réaliste expriment la ville moderne.

Dans Rue de Paris, temps de pluie, une perspective dramatique ; dans Les Raboteurs de parquet, la représentation réaliste du travail.
Caillebotte est un pionnier du paysage urbain qui fut à la fois peintre et mécène des artistes impressionnistes.`
  },

  // ===== Postimpressionnisme =====
  'vangogh': {
    name: 'Van Gogh',
    description: `Les touches en spirale et l\'empâtement épais de Van Gogh ont été appliqués.
Le contraste intense entre le bleu cobalt et le jaune chrome exprime directement l\'émotion intérieure.

Dans La Nuit étoilée, il tordit le ciel nocturne en spirales ; dans Les Tournesols, il embrasa le jaune de vie.
Van Gogh un art immortel créé dans une vie brève et intense de 37 ans.`
  },
  'cezanne': {
    name: 'Cézanne',
    description: `La structure géométrique et les touches constructives de Cézanne ont été appliquées.
La nature est simplifiée en cylindres, sphères et cônes ; les tons calmes révèlent l\'essence de la forme.

Il peignit La Montagne Sainte-Victoire plus de 60 fois en explorant la structure de la nature ; dans Pommes et Oranges, il reconstruisit la forme de la nature morte.
Appelé le père de l\'art moderne, Picasso et Braque découvrirent le cubisme dans son œuvre.`
  },
  'gauguin': {
    name: 'Gauguin',
    description: `Les contours audacieux et les couleurs pures en aplat de Gauguin ont été appliqués.
D\'épais contours noirs remplis de couleurs saturées créent des compositions exotiques et symboliques.

Quittant une vie stable d\'agent de change, il partit à Tahiti pour peindre Femmes de Tahiti et Le Christ jaune.
Gauguin est un peintre de l\'évasion qui poursuivit la beauté primitive en tournant le dos à la civilisation.`
  },

  // ===== Fauvisme =====
  'matisse': {
    name: 'Matisse',
    description: `Les couleurs pures audacieuses et les formes plates simplifiées de Matisse ont été appliquées.
Les primaires sont disposés avec audace et les formes sont simplifiées pour créer des compositions intensément décoratives.

Du rythme primordial de La Danse aux expérimentations de champ de couleur de L\'Atelier rouge, des huiles aux papiers découpés, il élargit les possibilités de la couleur tout au long de sa vie.
Matisse est un maître du Fauvisme qui n\'a jamais cessé d\'explorer les possibilités de la couleur jusqu\'à 84 ans.`
  },
  'derain': {
    name: 'Derain',
    description: `Les primaires explosifs et les touches rugueuses de Derain ont été appliqués.
Des couleurs vives comme directement sorties du tube sont appliquées avec rudesse sur la toile.

Des primaires irréels dans Le Pont de Londres, des aplats audacieux dans Bateaux à Collioure — avec Matisse, il donna naissance au Fauvisme dans le sud de la France.
Derain fut un guerrier qui brisa la tradition avec audace et alluma une révolution de la couleur.`
  },
  'vlaminck': {
    name: 'Vlaminck',
    description: `Les couleurs violentes et la touche rude de Vlaminck ont été appliquées.
La peinture épaisse appliquée avec agressivité dégage l\'énergie la plus intense parmi les fauves.

Des couleurs explosives dans Maisons à Chatou, une texture rugueuse dans Arbres rouges — de cycliste à peintre sans formation artistique.
Vlaminck est le peintre le plus fauve, qui embrasa la toile par le seul instinct, sans formation formelle.`
  },

  // ===== Expressionnisme =====
  'munch': {
    name: 'Munch',
    description: `Les lignes ondulantes distordues et les couleurs intensément psychologiques de Munch ont été appliquées.
Des lignes en spirale tordent les formes ; des ciels rouge sang et des jaunes maladifs expriment l\'angoisse.

Dans Le Cri, la terreur existentielle ; dans Madonna, la sensualité sombre — le trauma d\'avoir perdu sa mère à 5 ans et sa sœur à 14 fut la source de toute son œuvre.
Munch est un pionnier de l\'Expressionnisme qui peignit non ce que l\'on voit, mais ce que l\'on ressent.`
  },
  'kirchner': {
    name: 'Kirchner',
    description: `Les formes anguleuses et les primaires qui s\'entrechoquent de Kirchner ont été appliqués.
Des formes tordues et pointues et des couleurs qui se heurtent rudement expriment l\'angoisse et l\'aliénation urbaines.

Dans Scène de rue à Berlin, des foules acérées ; dans Cinq femmes dans la rue, des formes primitives.
Kirchner est Fondateur du groupe expressionniste allemand « Die Brücke », qualifié d\'art dégénéré par les nazis et qui mit fin à ses jours.`
  },
  'kokoschka': {
    name: 'Kokoschka',
    description: `Les touches violentes et les couleurs tranchantes de Kokoschka ont été appliquées.
Des touches dévastatrices et des tons fiévreux révèlent de façon explosive la psychologie des sujets.

Dans La Fiancée du vent, la passion amoureuse ; dans sa série d\'Autoportraits, l\'angoisse intérieure.
Kokoschka est un anatomiste de l\'intérieur, qualifié de dégénéré par les nazis avec 400 œuvres confisquées.`
  },

  // ===== Modernisme =====
  'picasso': {
    name: 'Picasso',
    description: `La déconstruction géométrique et la composition multi-perspective de Picasso ont été appliquées.
Les visages et les corps sont décomposés en plans angulaires, montrant face et profil simultanément.

Avec Les Demoiselles d\'Avignon, il ouvrit le Cubisme ; dans le Portrait de Dora Maar, il acheva la déconstruction multiperspective, laissant plus de 50 000 œuvres en 91 ans de vie.
Picasso est l\'art du XXe siècle en soi : peindre non comme on voit, mais comme on pense.`
  },
  'lichtenstein': {
    name: 'Lichtenstein',
    description: `Les points Ben-Day et les épais contours noirs de Lichtenstein ont été appliqués.
Les techniques d\'impression de bande dessinée agrandies en peinture ; des primaires en aplat et des trames de points transforment la culture populaire en art.

Dans Whaam!, des explosions d\'avions de combat ; dans Happy Tears, des émotions de bande dessinée sur la toile, provoquant le débat : « Est-ce de l\'art ? »
Lichtenstein est un maître du Pop Art qui effaça la frontière entre culture populaire et art.`
  },
  'miro': {
    name: 'Miró',
    description: `Les symboles ludiques et les combinaisons de couleurs primaires de Miró ont été appliqués.
Des lignes libres comme celles d\'un enfant et des formes de couleurs primaires flottent sur des fonds blancs.

Dans Le Paysage catalan, il dénoua les souvenirs de sa terre en symboles ; dans Vers les étoiles, il déploya l\'imagination cosmique.
Miró est un esprit libre de l\'art qui franchit les frontières des genres pour créer son propre monde de symboles.`
  },
  'magritte': {
    name: 'Magritte',
    description: `Les paradoxes philosophiques et les scènes impossibles de Magritte ont été appliqués.
Peint de manière réaliste, mais combiné de façons impossibles dans la réalité, détruisant les cadres de la perception.

Dans La Trahison des images, il questionna la nature de la représentation ; dans Golconde, il inversa le quotidien en surréalité.
Magritte est un peintre penseur qui poursuivit le mystère au-delà de ce que l\'œil peut voir.`
  },
  'chagall': {
    name: 'Chagall',
    description: `Les couleurs oniriques et la fantaisie poétique de Chagall ont été appliquées.
Des tons pastel doux et une atmosphère de rêve déploient des mondes d\'amour et de nostalgie.

Dans Moi et le Village, les souvenirs de son Vitebsk natal ; dans Au-dessus de la ville, son amour pour Bella — peints tout au long de sa vie.
Chagall est un poète sur toile qui chanta l\'amour et la nostalgie en couleur jusqu\'à 97 ans.`
  }
};

export default { movementsBasicInfo, movementsLoadingEducation, movementsResultEducation };
