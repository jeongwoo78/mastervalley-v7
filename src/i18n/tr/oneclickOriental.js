// ========================================
// Tek Tıkla Doğu Sanatı — Türkçe (tr)
// 2026-03-11
// ========================================

export const oneclickOrientalBasicInfo = {
  'korean-minhwa': {
    name: 'Kore Geleneksel Resmi',
    subtitle1: 'Minhwa (Halk Resmi)',
    subtitle2: 'Boşluk ve renk estetiği'
  },
  'korean-pungsokdo': {
    name: 'Kore Geleneksel Resmi',
    subtitle1: 'Pungsokdo (Tür Resmi)',
    subtitle2: 'Boşluk ve renk estetiği'
  },
  'korean-jingyeong': {
    name: 'Kore Geleneksel Resmi',
    subtitle1: 'Jingyeong Sansu (Gerçek Görünüm Manzarası)',
    subtitle2: 'Boşluk ve renk estetiği'
  },
  'chinese-ink': {
    name: 'Çin Geleneksel Resmi',
    subtitle1: 'Mürekkep Yıkama Resmi (Shuǐmò)',
    subtitle2: 'Mürekkep ve boşluğun felsefesi'
  },
  'chinese-gongbi': {
    name: 'Çin Geleneksel Resmi',
    subtitle1: 'Gongbi (Titiz Resim)',
    subtitle2: 'Mürekkep ve boşluğun felsefesi'
  },
  'japanese-ukiyoe': {
    name: 'Japon Geleneksel Resmi',
    subtitle1: 'Ukiyo-e (Geçici Dünya Resimleri)',
    subtitle2: 'Geçici dünyanın güzelliği'
  },
  'japanese-rinpa': {
    name: 'Japon Geleneksel Resmi',
    subtitle1: 'Rinpa (Dekoratif Okul)',
    subtitle2: 'Altın yaprak üzerinde açan dekoratif güzellik'
  }
};

export const oneclickOrientalPrimary = {
  content: `Batı doldurunca Doğu boşalttı.\nResmetmeyerek konuşmanın sanatı.\n\nKore mürekkep ve boşluk arasında,\nÇin manzara ve süsleme arasında,\nJapon\'da geçicilik ve ihtişam bir arada.\n\nÜç ülke, üç fırça darbesi.\nŞimdi sen burada duruyorsun.`
};

export const oneclickOrientalSecondary = {

  'korean-minhwa': {
    content: `Kore geleneksel resmi, boşluğu kucaklayan ve fırça aracılığıyla ruhu yakalayan bir sanattır.\nAydınlar mürekkeple, halk renkle boyadı.\n\nMinhwa, Joseon köylülerinin yaşama dair dileklerini dile getirdiği resimlerdir.\nKaplanlar, saksağanlar, şakayıklar ve diğer sembolik konular özgürce kompoze edildi.`
  },

  'korean-pungsokdo': {
    content: `Kore geleneksel resmi, boşluğu kucaklayan ve fırça aracılığıyla ruhu yakalayan bir sanattır.\nAydınlar mürekkeple, halk renkle boyadı.\n\nPungsokdo, Joseon köylülerinin günlük yaşamını yakalayan resimlerdir.\nGüreş, çamaşır yıkama ve köy okulu sahneleri canlı fırça darbeleriyle kayıt altına alındı.`
  },

  'korean-jingyeong': {
    content: `Kore geleneksel resmi, boşluğu kucaklayan ve fırça aracılığıyla ruhu yakalayan bir sanattır.\nAydınlar mürekkepte, halk renkle boyadı.\n\nJingyeong sansu, doğrudan gözlemle çizilen Kore\'nin gerçek dağlarının resimleridir.\nJeong Seon bu üslubu dikey doku darbeleri ve cesur kompozisyonlarla mükemmelleştirdi.`
  },

  'chinese-ink': {
    content: `Çin geleneksel resmi, mürekkep tonlamalarıyla evrenin ruhunu yakalayan bir sanattır.\nAydının ruhu ve sarayın tekniği bin yıl boyunca bir arada yaşadı.\n\nMürekkep yıkama resmi, dünyayı yalnızca mürekkep tonlamalarıyla betimleyen aydın sanatıdır.\nBoyasız boşluk en çok konuşandır.`
  },

  'chinese-gongbi': {
    content: `Çin geleneksel resmi, mürekkep tonlamalarıyla evrenin ruhunu yakalayan bir sanattır.\nAydının ruhu ve sarayın tekniği bin yıl boyunca bir arada yaşadı.\n\nGongbi, tek bir yanlış fırça darbesi olmaksızın titiz hassasiyetle yapılan saray resmidir.\nŞeffaf renkler derinlik oluşturmak için katman katman uygulanır.`
  },

  'japanese-ukiyoe': {
    content: `Japon geleneksel resmi, popüler duyarlılık ve saray ihtişamının bir arada yaşadığı bir sanattır.\nSokak baskıları ve altın dekoratif resim her biri kendi güzelliğiyle yeşerdi.\n\nUkiyo-e, Edo dönemi köylülerin zevkle tükettiği tahta blok baskılardır.\nCesur konturlar ve düz renk yüzeyleri İzlenimcilere ilham verdi.`
  },

  'japanese-rinpa': {
    content: `Japon geleneksel resmi, popüler duyarlılık ve saray ihtişamının bir arada yaşadığı bir sanattır.\nSokak baskıları ve altın dekoratif resim her biri kendi güzelliğiyle yeşerdi.\n\nRinpa, doğayı altın yaprak üzerine cömertçe süsleyen saray resim üslubudur.\nRenkler katman katman biriktirilir; çiçeklere ve dalgalara doğal, akıcı bir güzellik verir.`
  }
};

export default {
  oneclickOrientalBasicInfo,
  oneclickOrientalPrimary,
  oneclickOrientalSecondary
};
