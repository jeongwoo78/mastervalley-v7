// ========================================
// One-Click Lukisan Oriental — Bahasa Indonesia (id)
// 2026-03-11
// ========================================

export const oneclickOrientalBasicInfo = {
  'korean-minhwa': { name: 'Lukisan Tradisional Korea (Korean Traditional Painting)', subtitle1: 'Minhwa (Minhwa)', subtitle2: 'Harapan rakyat dalam gambar' },
  'korean-pungsokdo': { name: 'Lukisan Tradisional Korea (Korean Traditional Painting)', subtitle1: 'Pungsokdo (Pungsokdo)', subtitle2: 'Keseharian menjadi seni' },
  'korean-jingyeong': { name: 'Lukisan Tradisional Korea (Korean Traditional Painting)', subtitle1: 'Jingyeong Sansuhwa (Jingyeong Sansuhwa)', subtitle2: 'Gunung Joseon dilihat dengan mata sendiri' },
  'chinese-ink': { name: 'Lukisan Tradisional Tiongkok (Chinese Traditional Painting)', subtitle1: 'Tinta (Ink Wash)', subtitle2: 'Lanskap jiwa dalam tinta' },
  'chinese-gongbi': { name: 'Lukisan Tradisional Tiongkok (Chinese Traditional Painting)', subtitle1: 'Gongbi (Gongbi)', subtitle2: 'Puncak presisi' },
  'japanese-ukiyoe': { name: 'Lukisan Tradisional Jepang (Japanese Traditional Painting)', subtitle1: 'Ukiyo-e (Ukiyo-e)', subtitle2: 'Dunia mengambang terukir di atas kayu' },
  'japanese-rinpa': { name: 'Lukisan Tradisional Jepang (Japanese Traditional Painting)', subtitle1: 'Rinpa (Rinpa)', subtitle2: 'Estetika dekoratif di atas lembaran emas' }
};

export const oneclickOrientalPrimary = {
  content: `Ketika Barat mengisi, Timur mengosongkan.
Seni mengungkapkan tanpa melukis.

Korea, antara tinta dan kekosongan.
Tiongkok, antara lanskap dan ornamen.
Jepang, di mana yang fana dan yang megah berdampingan.

Tiga negara, tiga ujung kuas.
Kini, giliran Anda berhenti di sini.`
};

export const oneclickOrientalSecondary = {

  'korean-minhwa': {
    content: `Lukisan tradisional Korea adalah seni yang menghormati kekosongan dan menyimpan jiwa dalam kuas.
Para cendekiawan melukis dengan tinta; rakyat, dengan warna.

Minhwa adalah lukisan di mana rakyat Joseon mengungkapkan harapan hidup mereka.
Harimau, murai, peoni, dan motif simbolis lainnya disusun secara bebas.`
  },

  'korean-pungsokdo': {
    content: `Lukisan tradisional Korea adalah seni yang menghormati kekosongan dan menyimpan jiwa dalam kuas.
Para cendekiawan melukis dengan tinta; rakyat, dengan warna.

Pungsokdo adalah lukisan yang mencatat kehidupan sehari-hari orang biasa Joseon.
Gulat, mencuci pakaian, dan adegan di sekolah, ditangkap dengan goresan tangkas dan ringan.`
  },

  'korean-jingyeong': {
    content: `Lukisan tradisional Korea adalah seni yang menghormati kekosongan dan menyimpan jiwa dalam kuas.
Para cendekiawan melukis dengan tinta; rakyat, dengan warna.

Jingyeong sansuhwa adalah lukisan yang menggambarkan langsung gunung-gunung nyata Joseon.
Gyeomjae Jeong Seon menyempurnakannya dengan goresan vertikal dan komposisi berani.`
  },

  'chinese-ink': {
    content: `Lukisan tradisional Tiongkok adalah seni yang menangkap qi alam semesta melalui gradasi tinta.
Jiwa cendekiawan dan teknik istana berdampingan selama seribu tahun.

Lukisan tinta adalah seni cendekiawan yang melukis dunia hanya dengan gradasi tinta.
Ruang kosong yang tidak dilukis justru berkata paling banyak.`
  },

  'chinese-gongbi': {
    content: `Lukisan tradisional Tiongkok adalah seni yang menangkap qi alam semesta melalui gradasi tinta.
Jiwa cendekiawan dan teknik istana berdampingan selama seribu tahun.

Gongbi adalah lukisan istana dengan presisi sempurna, tanpa satu goresan pun yang melenceng.
Lapisan demi lapisan warna transparan ditumpuk untuk menciptakan kedalaman.`
  },

  'japanese-ukiyoe': {
    content: `Lukisan tradisional Jepang adalah seni di mana kepekaan rakyat dan kemegahan istana berdampingan.
Cetakan jalanan dan lukisan dekoratif berlapis emas masing-masing mengembangkan keindahannya.

Ukiyo-e adalah cetakan balok kayu yang dinikmati rakyat Edo.
Kontur tebal dan bidang warna datarnya menginspirasi para impresionis.`
  },

  'japanese-rinpa': {
    content: `Lukisan tradisional Jepang adalah seni di mana kepekaan rakyat dan kemegahan istana berdampingan.
Cetakan jalanan dan lukisan dekoratif berlapis emas masing-masing mengembangkan keindahannya.

Rinpa adalah lukisan istana yang menghiasi alam dengan kemegahan di atas lembaran emas.
Lapisan warna yang ditumpuk menciptakan difusi alami pada bunga dan gelombang.`
  }
};

export default {
  oneclickOrientalBasicInfo,
  oneclickOrientalPrimary,
  oneclickOrientalSecondary
};
