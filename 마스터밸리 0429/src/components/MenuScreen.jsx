...
const MenuScreen = ({ 
  ...
  // 흐름:
  //  - 평소(빠르게 응답 + 결제내역 X) → "복원할 구매 내역이 없습니다."
  //  - 평소(빠르게 응답 + 복원됨) → "구매가 복원되었습니다."
  //  - 모든 실패/지연/에러 → "결제가 진행 중입니다."
  const handleRestorePurchases = async () => {
    ...
    try {
      // 1. RevenueCat 영수증 재검증
      const restoreResult = await restorePurchases();
      
      if (!restoreResult.success) {
        // 모든 실패(pending, network, RC 에러 등) → 단일 메시지로 통합
      ...
      } else if (data.success || response.status === 402) {
        // 평소 케이스: 복원할 내역 없음 (이미 처리됐거나 결제 자체 없음)
        setRestoreToast('restoreNoItems');
      } else {
        // 그 외 모든 실패 → pending 메시지로 통합
        setRestoreToast('restorePending');
      }
      ...
      // 예외도 pending으로 통합 (사용자에게 "결제 진행 중" 안내 — 다시 시도 유도)
      setRestoreToast('restorePending');
      setTimeout(() => setRestoreToast(''), 3000);
    } finally {
      setRestoring(false);
