...
⭐ STYLE 2: ROMAN MOSAIC (로마 모자이크)
   ...
   - Technique: LARGE VISIBLE tesserae tiles 100mm, THICK DARK GROUT LINES between tiles
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ========================================
    // 비회화 매체 감지 (모자이크/대리석 — 유화 문구 충돌 방지)
    // ========================================
    ...
    const isSculptureStyle = ['classicalsculpture', 'sculpture'].includes(normalizedForTile);
    const isNonPaintMedia = isTileStyle || isSculptureStyle;
    ...
    // ========================================
    // 비회화 매체: 대전제에서 유화/사실적 문구 제거
    // 모자이크: 타일 텍스처, 대리석: 순백 대리석과 충돌 방지
    // ========================================
    ...
    if (isSculptureStyle) {
      coreRulesPrefix = coreRulesPrefix.replace(
        'Clean unsigned artwork, pure painted surface.',
        'Pure white marble surface with carved stone texture.'
      );
    }
    ...
    // 비회화 매체(모자이크+대리석): finalPrompt에서 유화/사실적 렌더링 유발 문구 제거
    if (isNonPaintMedia) {
      ...
      finalPrompt = finalPrompt.replace(/Skin always rendered warmer than warm brown[^,]*/gi, '');
      finalPrompt = finalPrompt.replace(/with visible warm undertones[^,]*/gi, '');
    }
    ...
    if (attractiveMode !== 'none') {
      ...
      if (attractiveMode === 'weak') {
        ...
        // 비회화 매체(모자이크/대리석): 유화식 매력 강화 스킵 (텍스처 충돌)
        if (!isNonPaintMedia) {
          finalPrompt = finalPrompt + attractiveEnhancement;
        }
        logData.prompt.applied.attractive = !isNonPaintMedia;
      }
    }
